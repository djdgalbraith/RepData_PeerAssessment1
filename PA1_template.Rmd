...
* The **mean** total number of steps taken per day is `r format(round(mean.steps, 0))`
* The **median** total number of steps taken per day is `r format(round(median.steps, 0))`
...
The 5-minute interval which, on average across all the days in the dataset, contains the maximum number of steps is `r strftime(maximal.average.interval, format="%H:%M")`
...
```{r}
activity.imputed <- transform(activity.data, steps = ifelse(is.na(activity.data$steps), average.steps.per.interval$steps[match(activity.data$interval, average.steps.per.interval$interval)], activity.data$steps))
```
...
* The **mean** total number of steps taken per day is `r format(round(imputed.mean.steps, 0))`
* The **median** total number of steps taken per day is `r format(round(imputed.median.steps,0))`

Both the mean and the median values are equal due to the imputation strategy of mean replacement which imputed 8 days of data with the average values for the interval in question.  This results in an increase of `r format(round(imputed.mean.steps - mean.steps, 0))` in the average number of daily steps, which amounts to a total increase of `r format(round(imputed.mean.steps * 8, 0))` steps across the measured period of `r length(daily.steps)` days.
